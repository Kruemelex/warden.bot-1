...
const TAG_BYTES = 16;
const ACTIVE_VERSION_ENV = 'WARDEN_DATA_ACTIVE_KEY_VERSION';
const LOOKUP_KEY_ENV = 'WARDEN_DATA_LOOKUP_KEY';

function parseVersion(value) {
    const version = Number(value ?? 1);
    if (!Number.isSafeInteger(version) || version < 1 || version > 65535) {
        throw new Error(`${ACTIVE_VERSION_ENV} must be an integer between 1 and 65535.`);
    }
    return version;
}
...
function getEncryptionKey(version) {
    return decodeKey(
        process.env[`WARDEN_DATA_ENCRYPTION_KEY_V${version}`],
        `WARDEN_DATA_ENCRYPTION_KEY_V${version}`,
    );
}

function getLookupKey() {
    return decodeKey(process.env[LOOKUP_KEY_ENV], LOOKUP_KEY_ENV);
}

function normalizeContext(value) {
    const context = String(value ?? '').trim();
    if (!/^[a-z0-9][a-z0-9:._-]{2,127}$/u.test(context)) {
        throw new Error('Application encryption requires a stable, namespaced context.');
    }
    return context;
}
...
function encryptJson(context, value) {
    ...
    const keyVersion = parseVersion(process.env[ACTIVE_VERSION_ENV]);
...
function isApplicationEncryptionConfigured() {
    const version = parseVersion(process.env[ACTIVE_VERSION_ENV]);
    return Boolean(
        String(process.env[`WARDEN_DATA_ENCRYPTION_KEY_V${version}`] ?? '').trim()
        && String(process.env[LOOKUP_KEY_ENV] ?? '').trim(),
    );
}

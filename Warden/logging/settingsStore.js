...
const TABLE_NAME = 'warden_logging_settings';
...
async function read(guildId) {
    ...
    const rows = await database.query(
        `SELECT * FROM ${TABLE_NAME} WHERE guild_id = ? LIMIT 1`,
        [String(guildId)],
    );
...
async function seed(guildId, channels) {
    ...
    await database.query(
        `INSERT IGNORE INTO ${TABLE_NAME} (
            ...
            error_log_channel_id, staff_log_channel_id,
            leaderboard_approval_channel_id
        ) VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [
            String(guildId), channels.general, channels.users,
            channels.messages, channels.error, channels.staff,
            channels.approvals,
        ],
    );
    return read(guildId);
...
async function update(guildId, patch, expectedRevision, updatedBy) {
    ...
    for (const [key, value] of Object.entries(patch.channels ?? {})) {
        ...
        if (!column) throw new Error(`Unknown Warden logging channel setting: ${key}`);
    ...
    const result = await database.query(
        `UPDATE ${TABLE_NAME} SET ${assignments.join(', ')}
    ...
    if (result?.affectedRows !== 1) {
        ...
        error.code = 'WARDEN_LOGGING_SETTINGS_CONFLICT';
        throw error;
    }
    return read(guildId);
}

...
const descriptors = createLeaderboardEditorDescriptors({ commitPendingEdits });
...
const descriptorEditor = createDescriptorModalEditor({
    ...
    beginSubmission: async ({ state }) => {
        ...
        const submission = await loadPendingSubmission(context.leaderboard, context.submissionId);
...
async function approveSubmission(interaction, leaderboard, submissionId) {
    const { submission, newlyApproved } = await approvePendingSubmission(leaderboard, submissionId);
    ...
    if (newlyApproved) await notifySubmitter(interaction, submission, true, leaderboard);
    if (refreshError) throw refreshError;
}

async function deleteSubmission(interaction, leaderboard, submission) {
    ...
    try {
        ...
        await deletePendingSubmission(leaderboard, submission.id, { expectedUserId: submission.user_id });
    }
    catch (deleteError) {
        let authoritative;
        try {
            authoritative = await loadSubmission(leaderboard, submission.id);
        }
        catch (reloadError) {
        ...
        if (!authoritative) {
            await notifySubmitter(interaction, submission, false, leaderboard);
            return;
        }
...
async function handleApprovalAction(interaction, leaderboard, action, submissionId) {
    ...
    try {
        ...
        const operationKey = leaderboard === 'ace' ? 'ace:resolution' : `${leaderboard}:${submissionId}`;
        await resolutionOperations.run(operationKey, async () => {
            const context = buildContext(interaction, leaderboard, submissionId);
            const submission = await loadSubmission(context.leaderboard, context.submissionId);
            ...
            if (action === 'approve') {
                ...
                await approveSubmission(interaction, context.leaderboard, context.submissionId);
            }
            else {
                const error = pendingError(submission);
                if (error) throw new Error(error);
                await deleteSubmission(interaction, context.leaderboard, submission);
            }
        });
    }
    catch (error) {
...
function parseApprovalCustomId(customId) {
    ...
    return { leaderboard: match[1], action: match[2], submissionId: Number(match[3]) };
}
